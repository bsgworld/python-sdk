...
with bsg_api.ApiClient(configuration) as api_client:
    ...
    api_instance = bsg_api.SendersTagApi(api_client)
    type = 'type_example' # str | Sender type to list

    try:
        # List of Senders
        api_response = api_instance.senders(type)
        print("The response of SendersTagApi->senders:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling SendersTagApi->senders: %s\n" % e)
```
